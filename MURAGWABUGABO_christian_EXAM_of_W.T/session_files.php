...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO session_files(file_id, session_id, user_id, therapist_id, file_name,file_url,timestamp) VALUES (?, ?, ?, ?, ?,?)");
    $stmt->bind_param("isssss", $file_id, $session_id, $user_id, $therapist_id,$file_name,$file_url,$timestamp);

    // Set parameters and execute
    $file_id = $_POST['driver_id'];
    $session_id = $_POST['user_id'];
    $user_id = $_POST['license_number'];
    $therapist_id = $_POST['car_model'];
    $file_name = $_POST['capacity'];
    $file_url = $_POST['capacit'];
    $timestamp = $_POST['capaci'];
...
<body>
    <center><h2>session_files Table</h2></center>
    <table border="3">
        <tr>
...
// Prepare SQL query to retrieve all session_files
$sql = "SELECT * FROM session_files";
$result = $connection->query($sql);

// Check if there are any session_files
if ($result->num_rows > 0) {
    // Output data for each row
    while ($row = $result->fetch_assoc()) {
        $file_id = $row['file_id']; // Fetch the file_id
        echo "<tr>
            ...
            <td><a style='padding:4px' href='delete_session_files.php?file_id=$file_id'>Delete</a></td> 
            <td><a style='padding:4px' href='update_session_files.php?file_id=$file_id'>Update</a></td> 
        </tr>";
    }
