...
<head>
  ...
  <title>therapist_schedule Page</title>
  <style>
    /* Normal link */
    a {
      padding: 10px;
...
<section>
   <h1><u>therapist_schedule Form</u></h1>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO therapist_schedule(session_id, therapist_id, client_id, start_time, end_time,session_type) VALUES (?, ?, ?, ?, ?,?)");
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>therapist_schedule DETAILS</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>therapist_schedule Table</h2></center>
    <table border="3">
        <tr> 
           <th>schedule_id</th>
            <th>therapist_id</th>
...
// Prepare SQL query to retrieve all therapist_schedule
$sql = "SELECT * FROM therapist_schedule";
$result = $connection->query($sql);

// Check if there are any therapist_schedule
if ($result->num_rows > 0) {
    // Output data for each row
    while ($row = $result->fetch_assoc()) {
        $schedule_id = $row['schedule_id']; // Fetch the schedule_id
        echo "<tr>
            ...
            <td><a style='padding:4px' href='delete_therapist_schedule.php?schedule_id=$schedule_id'>Delete</a></td> 
            <td><a style='padding:4px' href='update_therapist_schedule.php?schedule_id=$schedule_id'>Update</a></td> 
        </tr>";
    }

} else {
    echo "<tr><td colspan='8'>No data found</td></tr>"; // Corrected colspan to match the number of columns
}
