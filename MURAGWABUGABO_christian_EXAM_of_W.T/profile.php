...
<section>
  <h1><u>Profile Form</u></h1>

<form method="post" onsubmit="return confirmInsert();">

    <label for="profile_id">Profile ID:</label>
    <input type="number" id="profile_id" name="profile_id" required><br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO profile(profile_id, user_id, name, gender, date_of_birth,address) VALUES (?, ?, ?, ?, ? )");
    $stmt->bind_param("isssss", $profile_id, $user_id, $name, $gender,$date_of_birth,$address);

    
    // Set parameters and execute
    $profile_id = $_POST['driver_id'];
    $user_id = $_POST['user_id'];
    $name = $_POST['license_number'];
    $gender = $_POST['car_model'];
    $date_of_birth = $_POST['capacity'];
    $address = $_POST['capacit'];
...
<body>
    <center><h2>Profile Table</h2></center>
    <table border="3">
        <tr>
            <th>Profile_id</th>
            <th>User_id</th>
...
// Prepare SQL query to retrieve all Profile
$sql = "SELECT * FROM Profile";
$result = $connection->query($sql);

// Check if there are any Profile
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<tr>
            ...
            <td><a style='padding:4px' href='delete_profile.php?profile_id=$profile_id'>Delete</a></td> 
            <td><a style='padding:4px' href='update_profile.php?profile_id=$profile_id'>Update</a></td> 
        </tr>";
    }
